...
import org.springframework.cloud.openfeign.async.support.FeignHttpClientProperties;
...
public class AsyncHttpClient5FeignConfiguration {
	...
	public AsyncClientConnectionManager asyncHc5ConnectionManager(FeignHttpClientProperties httpClientProperties) {
		return PoolingAsyncClientConnectionManagerBuilder.create()
				.setTlsStrategy(clientTlsStrategy(httpClientProperties.isDisableSslValidation()))
				.setMaxConnTotal(httpClientProperties.getMaxConnections())
				.setMaxConnPerRoute(httpClientProperties.getMaxConnectionsPerRoute())
				.setConnPoolPolicy(
						PoolReusePolicy.valueOf(httpClientProperties.getAsyncHc5().getPoolReusePolicy().name()))
				.setPoolConcurrencyPolicy(PoolConcurrencyPolicy
						.valueOf(httpClientProperties.getAsyncHc5().getPoolConcurrencyPolicy().name()))
	...
	@Bean
	public CloseableHttpAsyncClient asyncHttpClient5(AsyncClientConnectionManager asyncClientConnectionManager,
			FeignHttpClientProperties httpClientProperties) {
		final IOReactorConfig ioReactorConfig = IOReactorConfig.custom()
				.setSoTimeout(Timeout.ofMilliseconds(httpClientProperties.getConnectionTimerRepeat())).build();
		asyncHttpClient5 = HttpAsyncClients.custom().disableCookieManagement().useSystemProperties() // Need
																										// for
																										// proxy
				.setVersionPolicy(
						HttpVersionPolicy.valueOf(httpClientProperties.getAsyncHc5().getHttpVersionPolicy().name())) // Need
				// for
				// proxy
				.setConnectionManager(asyncClientConnectionManager).evictExpiredConnections()
				.setIOReactorConfig(ioReactorConfig)
				.setDefaultRequestConfig(RequestConfig.custom()
						.setConnectTimeout(
								Timeout.of(httpClientProperties.getConnectionTimeout(), TimeUnit.MILLISECONDS))
						.setResponseTimeout(Timeout.of(httpClientProperties.getAsyncHc5().getResponseTimeout(),
								httpClientProperties.getAsyncHc5().getResponseTimeoutUnit()))
	...
	private TlsStrategy clientTlsStrategy(boolean isDisableSslValidation) {
		...
		if (isDisableSslValidation) {
			try {
				...
				disabledSslContext.init(null,
						new TrustManager[] { new AsyncHttpClient5FeignConfiguration.DisabledValidationTrustManager() },
						new SecureRandom());
